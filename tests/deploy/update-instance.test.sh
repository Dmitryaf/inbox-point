...
if [[ ${1:-} == compose ]]; then
  case " $* " in
    ...
    *' --profile operations run -T --no-deps --name '*' --entrypoint /bin/sh backup -c '*)
      ...
      printf '{"closedRequestRetentionDays":7,"createdAt":"2026-09-14T12:00:00.000Z","expiresAt":"2026-09-21T12:00:00.000Z","files":[%s],"formatVersion":1,"instanceId":"test","secretsIncluded":false,"sqliteSchemaVersion":11}\n' \
